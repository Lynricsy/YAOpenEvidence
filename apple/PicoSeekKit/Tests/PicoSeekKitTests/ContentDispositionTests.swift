...
struct ContentDispositionTests {
    ...
    /// 取错分支用户拿到的就是 `PicoSeek-<id>.pdf` 这种无意义文件名。
    @Test("同时存在时取 RFC 5987 的 filename* 并百分号解码")
    func prefersExtendedFilename() {
        let header = """
        attachment; filename="PicoSeek-a1b2.pdf"; \
        filename*=UTF-8''PicoSeek-20260919-%E4%B9%B3%E8%85%BA%E7%99%8C%E7%9A%84%E9%9D%B6%E5%90%91%E6%B2%BB%E7%96%97.pdf
        """
        #expect(APIClient.filename(fromContentDisposition: header) == "PicoSeek-20260919-乳腺癌的靶向治疗.pdf")
    }
