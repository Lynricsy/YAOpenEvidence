...
describe('筛选请求契约', () => {
  it('年份互斥且未启用分区时不发送未收录开关', () => {
    ...
    expect(recent).not.toHaveProperty('year_from')
    expect(recent).not.toHaveProperty('year_to')
    expect(recent).not.toHaveProperty('keep_unranked')
    const f = {
      ...DEFAULT_FILTERS,
      yearMode: 'range' as const,
      yearFrom: 2020,
      yearTo: 2024,
      quartiles: [1, 2],
      keepUnranked: true,
    ...
    expect(range).not.toHaveProperty('years')
    expect(range).not.toHaveProperty('use_paywall')
