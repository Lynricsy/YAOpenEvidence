...
import { RotateCcw, SlidersHorizontal, ChevronDown, Info } from 'lucide-react'
import {
  DEFAULT_FILTERS,
  describeFilters,
  type FilterState,
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-5">
          ...
          <div className="space-y-3">
            {[1, 2, 3, 4].map((q) => (
              <label key={q} className="flex cursor-pointer items-center gap-3">
                <Checkbox
                  aria-label={q + '区 Q' + q}
                  checked={f.quartiles.includes(q)}
                  onCheckedChange={(checked) =>
                    update({
                      quartiles: checked
                        ? [...f.quartiles, q].sort()
                        : f.quartiles.filter((v) => v !== q),
                    })
                  }
                />
                <span
                ...
                />
                <span className="text-sm">{q}区</span>
        ...
        <section className="space-y-4 border-b py-5">
          <h3 className="text-xs font-semibold text-muted-foreground">年份</h3>
        ...
        <Collapsible className="py-5">
          <CollapsibleTrigger className="flex w-full items-center justify-between text-xs font-semibold text-muted-foreground">
            高级
          ...
          <CollapsibleContent className="space-y-6 pt-5">
            <div className="space-y-4">
              <div className="flex items-center justify-between text-xs">
                阅读篇数
                <span className="font-mono text-primary">{f.papers} 篇</span>
              </div>
              <Slider
                aria-label="阅读篇数"
                min={1}
            ...
            <div className="space-y-4">
              <div className="flex justify-between text-xs">
                附加知识库命中<span className="font-mono">{f.kbHits}</span>
              </div>
              <Slider
                aria-label="附加知识库命中"
                min={0}
      ...
      <p
        className="border-t bg-muted/50 px-5 py-4 text-xs leading-6 text-muted-foreground"
