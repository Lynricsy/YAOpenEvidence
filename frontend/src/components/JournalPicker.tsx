  ...
  return (
    <div className="space-y-3">
      ...
      >
        {presets.map(([label, v]) => (
          <ToggleGroupItem
            className="rounded-md border text-xs data-[state=on]:border-primary/30 data-[state=on]:bg-primary/10 data-[state=on]:text-primary"
            key={v}
            value={v}
          >
            {label}
      ...
      {custom.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {custom.map((v) => (
            <span
              className="inline-flex max-w-full items-center gap-1 rounded border bg-muted pl-2 text-xs"
              key={v}
            >
