...
const links = [
  { to: '/', label: '提问', icon: MessageSquare },
...
export function AppShell() {
  ...
  const navigation = (mobile = false) =>
    links
      .filter((link) => isAdmin || link.to !== '/admin/users')
      .map(({ to, label, icon: Icon }) => (
        <NavLink
          ...
          className={({ isActive }) =>
            (mobile ? 'mobile-nav ' : 'top-nav ') + (isActive ? 'active' : '')
          }
        >
        ...
        <div className="flex min-w-0 items-center gap-3">
          <Sheet open={menuOpen} onOpenChange={setMenuOpen}>
            ...
            <SheetContent side="left">
              <SheetHeader>
                <SheetTitle>YAOpenEvidence</SheetTitle>
              </SheetHeader>
              <nav className="grid gap-1 px-3">{navigation(true)}</nav>
            </SheetContent>
          </Sheet>
          <Link to="/" className="brand">
            ...
            />
            <span>
              YAOpen<span className="text-primary">Evidence</span>
            </span>
          </Link>
        </div>
        <nav
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-52">
              <div className="flex items-center justify-between gap-2 px-2 py-3 text-sm">
                ...
                <Badge variant="secondary">{isAdmin ? '管理员' : '用户'}</Badge>
              </div>
              <DropdownMenuSeparator />
