...
function Verification({ verified }: { verified?: boolean }) {
  return (
    <Badge
      variant="outline"
      className={verified ? 'status-green' : 'status-amber'}
    >
  ...
  return (
    <>
      <SheetHeader className="max-h-[35dvh] shrink-0 overflow-y-auto border-b pr-14">
        <SheetTitle className="text-base leading-6 break-words">
          [{n}] {paper?.title || '第 ' + n + ' 篇文献'}
        </SheetTitle>
        <SheetDescription className="sr-only">
          第 {n} 篇文献的原文、阅读笔记、核实引文与事实
        </SheetDescription>
        {paper && (
          <>
      ...
      {pending ? (
        ...
        >
          <TabsList
            className="mx-4 mb-3 grid w-auto shrink-0"
            style={{
              gridTemplateColumns: fallbackEnabled
                ? '1fr'
          ...
          >
            ...
            {!fallbackEnabled && (
              <>
                <TabsTrigger value="notes">阅读笔记</TabsTrigger>
                <TabsTrigger value="quotes">核实引文</TabsTrigger>
                ...
                <>
                  ...
                  <TabsContent value="quotes" className="min-w-0">
                    ...
                    {verifiedQuotes.map((quote, index) => (
                      ...
                      >
                        <div className="flex flex-wrap items-center gap-2">
                          <Verification verified={quote.verified} />
                          {quote.key_finding && (
                        ...
                        <blockquote className="border-l-2 border-primary/40 pl-3 text-sm leading-7 whitespace-pre-wrap">
                          {quote.quote}
                        </blockquote>
                      ...
                      >
                        <div className="flex flex-wrap items-center gap-2">
                          <Badge variant="secondary">
                            ...
                            )[fact.kind ?? 'finding'] ?? fact.kind}
                          </Badge>
                          <Verification verified={fact.verified} />
                        </div>
                        ...
                        {fact.quote && (
                          <blockquote className="border-l-2 pl-3 text-sm leading-7 whitespace-pre-wrap text-muted-foreground">
                            {fact.quote}
                          </blockquote>
                        )}
                        <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
  ...
  return (
    ...
    >
      <SheetContent
        ...
        className="h-dvh w-full gap-3 overflow-hidden sm:max-w-3xl"
      >
        <SheetClose asChild>
          <Button
            variant="ghost"
