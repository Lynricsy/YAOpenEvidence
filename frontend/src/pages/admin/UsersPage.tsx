    ...
    >
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription>设置新用户的账号与权限。</DialogDescription>
        </DialogHeader>
        <form
          className="space-y-5"
          onSubmit={(event) => {
            event.preventDefault()
            if (!create.isPending && usernameValid && passwordValid)
              create.mutate()
          }}
        >
          <div className="space-y-2">
            ...
            >
              3–64 个字符，以字母或数字开头，仅含字母、数字、下划线、点或连字符
            </p>
          </div>
          <div className="space-y-2">
          ...
          <DialogFooter>
            ...
            <Button
              type="submit"
              disabled={create.isPending || !usernameValid || !passwordValid}
            >
              {create.isPending ? (
...
function ResetPasswordDialog({
  ...
  const reset = useMutation({
    gcTime: 0,
    mutationFn: () =>
      resetUserPassword({ id: user.id, new_password: password }),
    onSuccess: () => {
      setPassword('')
      toast.success('已重置，该用户所有会话已失效')
      onClose()
    },
  })
  return (
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription className="break-all">
            用户：{user.username}。重置后，该用户所有会话将失效。
          </DialogDescription>
        </DialogHeader>
        <form
          className="space-y-5"
          onSubmit={(event) => {
            event.preventDefault()
            if (!reset.isPending && valid) reset.mutate()
          }}
        >
          <div className="space-y-2">
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={reset.isPending || !valid}>
              {reset.isPending ? (
                <Loader2 className="animate-spin" />
              ) : (
                <KeyRound />
