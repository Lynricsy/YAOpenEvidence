...
export default function KbSearchPage() {
  ...
  return (
    <div className="page min-w-0">
      ...
      <section className="space-y-4 border-b pb-6">
        {stats.isPending ? (
          ...
          <>
            <dl className="flex flex-wrap gap-x-10 gap-y-4">
              <div>
                <dt className="metadata">条目</dt>
                <dd className="mt-1 text-2xl font-semibold tabular-nums">
                  {stats.data.items.toLocaleString()}
                </dd>
              </div>
              <div>
                <dt className="metadata">论文</dt>
                <dd className="mt-1 text-2xl font-semibold tabular-nums">
                  {stats.data.papers.toLocaleString()}
                </dd>
              </div>
              {Object.entries(stats.data.by_kind).map(([name, count]) => (
                <div key={name}>
                  <dt className="metadata">{kindLabel(name)}</dt>
                  <dd className="mt-1 text-2xl font-semibold tabular-nums">
        ...
        {jobId && (
          <div className="space-y-3 border-t pt-4" aria-live="polite">
            ...
            {active && progress?.current != null && progress.total != null && (
      ...
      <form
        className="toolbar mt-6 flex flex-wrap items-end gap-3"
        onSubmit={(event) => {
      ...
      >
        ...
        <div className="space-y-2">
          <span className="field-label" id="kb-kind">
            类型
          </span>
          <Select value={kind} onValueChange={setKind}>
            <SelectTrigger aria-labelledby="kb-kind" className="w-28">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <div className="space-y-2">
          <span className="field-label" id="kb-count">
            条数
          </span>
          <Select value={topK} onValueChange={setTopK}>
            <SelectTrigger aria-labelledby="kb-count" className="w-24">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {['5', '8', '15', '30'].map((value) => (
        ...
        <Button type="submit" disabled={!q.trim() || results.isFetching}>
          <Search />
          检索
        </Button>
      </form>
        ...
        (results.isFetching ? (
          ...
          <div>
            {results.data.items.map((hit, index) => (
              <article className="list-row min-w-0 space-y-3" key={index}>
                <div className="flex flex-wrap items-center gap-2">
                  <Badge variant="outline">{kindLabel(hit.kind)}</Badge>
                  {hit.fact_kind && (
                    <Badge variant="outline">{kindLabel(hit.fact_kind)}</Badge>
                  )}
                  <Verified value={hit.verified} />
                ...
                {hit.quote && (
                  <blockquote className="break-words border-l-2 pl-4 text-sm leading-7 text-muted-foreground">
                    {hit.quote}
                  </blockquote>
                )}
