      ...
      {paper.isPending ? (
        ...
        <>
          <header className="space-y-4 border-b pb-6">
            <h1 className="break-words text-xl font-semibold leading-8">
              {paper.data.title || key}
            </h1>
            <p className="metadata break-words">{paper.data.authors}</p>
          ...
          <Tabs value={tab} onValueChange={setTab} className="mt-5 min-w-0">
            <TabsList>
              <TabsTrigger value="fulltext">全文</TabsTrigger>
              <TabsTrigger value="facts">事实</TabsTrigger>
            </TabsList>
            <TabsContent value="fulltext" className="min-w-0 pt-4">
              {fulltext.isPending ? (
                <Loading />
              ) : fulltext.isError ? (
                <QueryError error={fulltext.error} retry={fulltext.refetch} />
              ) : fulltext.data ? (
            ...
            <TabsContent value="facts" className="min-w-0 pt-4">
              {facts.isPending ? (
                ...
                facts.data.items.map((fact, index) => (
                  <article className="list-row space-y-3" key={index}>
                    <div className="flex flex-wrap items-center gap-2">
                      <Badge variant="outline">
                        {kindLabel(fact.kind ?? 'finding')}
                      </Badge>
                      <Verified value={fact.verified ?? false} />
                    ...
                    {fact.quote && (
                      <blockquote className="border-l-2 pl-4 text-sm leading-7 text-muted-foreground">
                        {fact.quote}
                      </blockquote>
                    )}
                  </article>
