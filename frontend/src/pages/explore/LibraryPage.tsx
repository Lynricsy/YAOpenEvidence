    ...
    <div className="page min-w-0">
      ...
      {papers.isPending ? (
        ...
        <>
          <div>
            {papers.data.items.map((paper) => (
              <article key={paper.key} className="list-row min-w-0 space-y-3">
                <Link
                  className="block break-words font-semibold leading-7 hover:text-primary"
                ...
                <div className="metadata flex flex-wrap gap-x-4 gap-y-1">
                  <span>
        ...
        <EmptyState
          ...
          description={
            q || offset ? '没有符合当前条件的文献。' : '完成一次问答后自动入库'
          }
        />
      )}
    </div>
