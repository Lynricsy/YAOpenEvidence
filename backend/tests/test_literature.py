...
def test_auto_falls_back_to_pubmed(client, monkeypatch):
    ...
    response = client.get("/v1/literature/search?q=x", headers=auth(USER_TOKEN))
...
def test_epmc_fulltext_failure_is_not_missing(client, upstream_http, ident):
    ...
    upstream_http(handle)
    response = client.get("/v1/literature/fulltext", params={"ident": ident}, headers=auth(USER_TOKEN))
    assert response.status_code == 502
    assert response.json()["code"] == "upstream_unavailable"
...
def test_identifiers_reach_each_literature_resource(client, upstream_http, ident, resource):
    ...
    upstream_http(handle)
    response = client.get(f"/v1/literature/{resource}?ident={ident}", headers=auth(USER_TOKEN))
    assert response.status_code == 200
    body = response.json()
...
def test_doi_fallback_preserves_failure(client, upstream_http, s2_status, epmc_status, expected):
    ...
    upstream_http(handle)
    response = client.get("/v1/literature/resolve?ident=10.1000/example", headers=auth(USER_TOKEN))
...
def test_fulltext_citation_failure_is_upstream_error(client, upstream_http):
    ...
    upstream_http(handle)
    response = client.get("/v1/literature/fulltext?ident=PMC123", headers=auth(USER_TOKEN))
    assert response.status_code == 502
...
def test_ident_is_required_and_nonempty(client, resource):
    for query in ("", "?ident="):
        response = client.get(f"/v1/literature/{resource}{query}", headers=auth(USER_TOKEN))
        assert response.status_code == 422
        assert response.json()["code"] == "validation_error"
...
def test_old_identifier_paths_are_removed(client, suffix):
    response = client.get(f"/v1/literature/10.1000%2Fexample{suffix}", headers=auth(USER_TOKEN))
    assert response.status_code == 404
