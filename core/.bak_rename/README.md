...
```bash
cd /data1/qyy/smk
./smk start          # 启动 vLLM + LiteLLM（tmux 后台，模型加载约 1–3 分钟）
./smk status         # 等到 vLLM 和 LiteLLM 都列出模型名
./smk ask "SGLT2抑制剂对HFpEF患者有什么获益？"
```
...
常用参数：
```bash
./smk ask --papers 12 "..."      # 多读几篇（默认 8 篇）
./smk ask --no-paywall "..."     # 不使用机构订阅下载
./smk stop                       # 演示结束后停止服务
```
...
| 命令 | 作用 |
|---|---|
| `./smk start [14b\|4b] [GPU]` | 启动 vLLM + LiteLLM，默认 Qwen3-14B 在 GPU 2；显存紧张用 `./smk start 4b 1` |
| `./smk stop` / `status` / `logs` | 停止 / 查看状态 / 打开 tmux 看服务日志 |
| `./smk ask "问题"` | **一键流水线**（推荐演示用，每次都真实检索 + 读全文，结果确定） |
| `./smk codex ["问题"]` | Codex 交互式 agent：模型自己决定调哪些工具，适合追问（"把 [2] 的摘要贴出来"、"读一下 [3] 的 Results"） |
| `./smk paywall get DOI` | 用机构订阅登录态下载付费墙全文到 `pdfs/` |
| `./smk verify` | 核对最近一次 Codex 回答里的 PMID/DOI 是否都来自工具返回（防编造） |
| `./smk test` | 不经过模型，直接测试检索工具是否可用 |
...
2. Codex 模式下，运行输出里必须有 `mcp: semantic_scholar/xxx (completed)`；之后跑 `./smk verify`，不在工具返回里的 PMID/DOI 会标 `SUSPECT`。
...
# 服务器上验证：
./smk paywall get 10.1016/j.jacc.2023.10.021
```
之后 `./smk ask` 自动启用（每次最多下载 `PAYWALL_MAX_PER_RUN`=5 篇，串行、间隔 4–9 秒）。
...
- `LOCAL_QWEN_KEY` 需等于 `litellm_config.yaml` 的 `master_key`（`smk` 已默认设置）
...
| 现象 | 处理 |
|---|---|
| `./smk status` 显示 not ready | 等 1–3 分钟；`./smk logs` 看 vLLM 是否还在加载 / 显存不足 |
| 显存不够 | `./smk stop && ./smk start 4b <空闲GPU>` |
| 检索无结果 / 429 | `./smk test` 直接测工具；配置 `S2_API_KEY` |
