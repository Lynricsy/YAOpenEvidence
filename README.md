...
流水线保留现有逐阶段策略：检索、抽取显式关闭思考，综合阶段开启思考；Codex 使用服务端默认思考。综合阶段输出额度为 16384 tokens，由思考与正文共享；原先的 2800 tokens 可能在生成正文前就被思考耗尽。模型返回空正文或 `finish_reason=length` 时，任务明确失败，不会保存为完成答案，也不会用相同预算反复重试。262144 是输入与输出共享上限，不代表两路满长度请求能并发，也不代表已完成医学领域精度评估。
